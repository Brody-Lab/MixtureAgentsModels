"""
    DeltaClicksZ{Q<:AbstractVector,B<:AbstractVector,C<:Symbol} <: Agent 

Agent corresponding to a simple delta rule for right and left clicks on the Poisson clicks task.
Assumes right choices as 1 and left choices as 2. Z-scored version.
...
"""
@with_kw struct DeltaClicksZ{Q<:AbstractVector,B<:AbstractVector,C<:Symbol} <: Agent 
...
"""
function next_Q!(Q::T,θ::DeltaClicksZ,data::D,t::Int) where {T <: AbstractArray, D <: RatData}
    @unpack zleftclicks,zrightclicks = data 

    if t < length(zrightclicks)
        Q[1] = zrightclicks[t+1]  
        Q[2] = zleftclicks[t+1]
    else
        Q[1] = 0
        Q[2] = 0
...
"""
function βtitle(θ::DeltaClicksZ)
    return "β(ΔClicksZ)"
end
...
"""
function agent2string(θ::DeltaClicksZ)
    return "DeltaClicksZ"
end


function atick(θ::DeltaClicksZ)
    return "ΔClicksZ"
end
