...
Parameters for an agent computing values as a model-free choice kernel from an Ito & Doya model-based reinforcement learning framework
...
"""
@with_kw struct MFchoice{T<:Real,Q<:AbstractVector,I<:Int,A<:Distribution,B<:AbstractVector,C<:Symbol} <: Agent 
...
"""
function next_Q!(Q::AbstractArray,θ::MFchoice,data::D,t::Int) where D <: RatData
    @unpack α = θ
    @unpack choices = data
    Q .*= (1 - α)
    Q[choices[t]] += α
...
"""
function agent2string(θ::MFchoice)
    return "MFchoice"
end
function atick(θ::MFchoice)
    return "MFc"
end