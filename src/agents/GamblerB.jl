...
@with_kw struct GamblerB{T<:Real,Q<:AbstractVector,I<:Int,A<:Distribution,B<:AbstractVector,C<:Symbol} <: Agent
...
"""
function next_Q!(Q::AbstractArray,θ::GamblerB,data::D,t::Int) where D <: RatData
    @unpack α = θ
...
"""
function agent2string(θ::GamblerB)
    return "GamblerB"
end

function atick(θ::GamblerB)
    return "GF"
end
