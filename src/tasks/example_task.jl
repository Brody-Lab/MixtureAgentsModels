"""
    EXAMPLEdata{T1,T2,T3,T4} <: RatData

Example behavioral data struct. Replace EXAMPLE with task name when creating for a specific task. Required fields are ones that must be included for models to run correctly. Optional fields can be added for agent compatibility. Check an agents `next_Q!` function to see what behavior data it requires.
...
"""
@with_kw struct EXAMPLEdata{I<:Int,VI<:AbstractVector{Int},VB<:AbstractVector{Bool},VA<:AbstractVector} <: RatData 
    ntrials::I
    nfree::I = ntrials
    choices::VI
    rewards::VI
...
"""
    EXAMPLEdata(data::D) where D <: Union{Dict,JSON3.Object}

Converts a dictionary or JSON object to a EXAMPLEdata struct.
"""
function EXAMPLEdata(data::D) where D <: Union{Dict,JSON3.Object}
...
"""
@with_kw struct EXAMPLEsim{T} <: SimOptions where T
    nsess::T = 1
    ntrials::T = 1000
    mean_ntrials::T = 0
...
"""
function simulate_task(model::M,agents::Array{A},sim_options::EXAMPLEsim,new_sess::AbstractArray{Bool}) where {M <: MixtureAgentsModel, A <: Agent}
...
If it is a .mat file, you can use `matread` to load the data, and then transform it into variable types required by EXAMPLEdata struct. 
You can also use `CSV.read` to load a .csv file into a DataFrame, and then convert it to a dictionary for EXAMPLEdata struct.
See "data/example_task_data.csv" for an example of how to format a .csv file for EXAMPLEdata struct, and "data/example_task_data.mat" for an example of how to format a .mat file for EXAMPLEdata struct.
"""
function load_EXAMPLE(file::String="data/example_task_data.csv")
    ext = split(file,'.')[end] # get file extension

    if ext == "mat"
        return load_EXAMPLE_mat(file)

    elseif ext == "csv"
        return load_EXAMPLE_csv(file)
    end
...
function load_EXAMPLE_csv(file="data/example_task_data.csv")
    # load into dataframe
